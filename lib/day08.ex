defmodule Day08 do
  @moduledoc """
  Addresses the puzzles on [Advent of Code//Day 8](https://adventofcode.com/2018/day/8). 
  ...
  * `D`, which has `0` child nodes and `1` metadata entry (`99`).
  ...
  If a node has _no child nodes_, its value is the sum of its metadata 
  ...
  * Node `A` has three metadata entries: `1`, `1`, and `2`. The `1` references node
    ...
    the value of node `A` is `33+33+0=_66_`.
  ...
  def values([0, entries | tail]) do
    {a,b} = tail |> Enum.split(entries)
    {a |> Enum.sum, b}
  end

  def values([children, entries | tail]) do
    1..children |> Enum.reduce(
      {Map.new, tail}, 
      fn index, {map, remaining} -> 
        {a, b} = values(remaining)
        {Map.put(map, index, a), b}
      end
    )
    |> (fn {map, remaining} -> 
      {remaining
      |> Enum.take(entries)
      |> Enum.reduce(0, fn index, acc -> acc + Map.get(map, index, 0) end),
      Enum.drop(remaining, entries)}
     end).()
  end

  def sumup([0, entries | tail]) do
    tail |> Enum.split(entries)
  end

  def sumup([children, entries | tail]) do
    1..children |> Enum.reduce(
      {[], tail}, 
      fn _, {values, remaining} -> 
        {a, b} = sumup(remaining)
        {values ++ a, b}
      end
    )
    |> (fn {values, remaining} -> {values ++ Enum.take(remaining, entries), Enum.drop(remaining, entries)} end).()
  end
end
